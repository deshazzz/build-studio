...
$("#dateForm").on('submit', function(e) {
    ...
    var data = {
        ...
        email: $("#dfEmail").val(),
        phone: $("#dfPhone").val(),
        preferredCommunication: $("#dfPreferred").val(),
        businessName: $("#dfBusinessName").val(),
        businessDomain: $("#dfBusinessDomain").val(),
        message: $("#dfMessage").val()
    };

    if ( isValidEmail(data['email']) && (data['name'].length > 1) && (data['businessName'].length > 1) && (data['businessDomain'].length > 1) && (data['preferredCommunication'].length > 1) && (data['message'].length > 1) && isValidPhoneNumber(data['phone']) ) {
        $.ajax({
            type: "POST",
